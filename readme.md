...
> **那本不该被翻烂的《百职录》**
>
> 当每秒产生上千张查询卷轴、反复读取同一份材料聚合详情时，为什么“每次都 JOIN 与聚合 MySQL”这条诚实的旧规矩会失效？Cache-Aside 又究竟替系统挡住了什么？
...
2. 选择 Direct、查询潮汐和魔法通路模式并开始实验。室外只显示受理状态并自动跟随查询卷轴进入店内；在 `/lab` 上方通过 SSE 观察服务端 SQL、连接池与缓存状态，最终对比表分别冻结目标/实际速率、目标完成率、通路数、实际请求延迟、需求侧延迟、Socket Errors 和 Error Rate。购买实验始终独立创建 150 个唯一购买请求，不由查询 QPS 或本轮完成请求数换算。
...
| 魔法通路 | wrk2 `connections`，即 HTTP 持久连接数 |
