...
首页是“暖潮街 17 号”的双店室外入口。点击左侧直接进入 `/seckill-lab`，页面会调用真实
...
1. 从首页进入材料情报店（`/material-shop`），围绕唯一实验货物 `ARC-004 · 星髓` 直接选择“配置查询潮汐”或“配置购买实验”。前厅不再提供单次查询分支；两个入口共享材料语境，但互不构成启动前置条件。由于共用库存与缓存，不应并行运行。组成材料只参与详情 JOIN，不作为独立商品开放。
...
材料店到实验室的视觉转场不承担启动语义。店外只把路径、目标速率、连接模式和固定时长
保存到同标签页的待运行计划并写入 URL；店内确认页面可见且全局指标 SSE 已收到首帧后，
...
| 路径 | 方案 |
|---|---|
| `GET /api/seckill/materials` | 当前唯一限量材料 `ARC-004 · 星髓`；不返回实时库存 |
