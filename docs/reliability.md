...
平级实验详情入口，不再经过单次查询或第二层方式选择。查询分支用两张可切换的解说卡介绍 Direct 与 Cache-Aside 的工程行为、缓存击穿/穿透风险；卡片只切换右侧方案说明，不代表运行路径已经选定。详情页同时展示实验对象，并允许设置两轮共同的查询潮汐、通路模式和固定时长，真正的本轮读取路径仍在实验室选择。该页面只保存共同条件并转场，不调用任务创建接口，也不重置购买实验库存。自动连接数在详情页只以 Direct 作为预估请求样本，进入实验室选定路径后会用同一估算接口重新计算，任务创建时才锁定最终值；手动连接数则作为两条路径共享条件直接带入。
`/lab` 先让用户选择 Direct 或 Cache-Aside，再等待页面可见并收到全局指标 SSE 首帧；观测就绪后仍不自动创建任务，只有用户明确点击店内开始按钮才提交创建请求，随后订阅任务 SSE、轮询恢复状态、显示详细指标和日志、执行停止，
...
查询结果的购买快捷入口也先返回这一步，不读取查询 QPS、连接数或结果作为购买启动条件。
业务目录、前端映射与新旧数据卷都只保留星髓；组成材料只用于详情 JOIN，不是可独立查询或购买的商品。
这样室外入口保持轻量，刷新或重新入店仍能从各自权威状态恢复。
...
- 店外到店内的 CSS 转场不驱动 Runner。店外只把待运行计划写入 sessionStorage 和 URL；店内至少获得一次可见绘制机会并收到 `/api/metrics/stream` 的首个快照后，才启用“开始压测”按钮。只有用户明确点击该按钮才提交 `POST /api/loadtests`；不支持 EventSource 时，以成功快照和已启用的持续轮询作为观测就绪条件。
...
- 热点击穿与缓存穿透沿用同一条任务 SSE，不新增逐请求 trace。场景动画只由带事件 ID 的真实 `metric`、`cache_evicted`、`cache_rebuilt`、`cache_recovered` 和终态事件推进；全局指标 SSE 与任务 GET/轮询只恢复静态证据，不能用累计差值猜测或补画路径。已接收的事件队列不依赖 Runner 继续处于 `running`，断线回放遇到终态时也必须先演完前序步骤，再把 `completed` 作为队尾证据。三个缓存里程碑携带发布时冻结的任务指标副本，历史回放不会被后续采样改写；`cache_evicted` 的事件类型与 Runner 控制响应才是 DEL 的权威证据，它携带的指标仍可能是 DEL 前最后一帧，删除后的 Key 状态由下一条 `metric` 证明。热点击穿按“真实 DEL → MISS/互斥合并回源 → DTO 重建 → 三秒稳定窗口”闭环；缓存穿透把正常 DTO MISS、MySQL 不存在查询、负缓存写入与负缓存命中分别展示，负缓存保护还用 `coalescedAfterMiss` 区分锁外首查命中和互斥后二查命中，任何负缓存命中都不得冒充正常 DTO HIT。
...
左侧不使用同一轮持续压测同时证明限流和库存正确性：300 份库存会很快售罄，继续发流量只能
主要观察售罄，无法解释正常准入链路。当前页面以库存正确性、入口过载保护两个问题为主；顶部链路与单次请求作为辅助入口，两个主实验在桌面并列：

- 单次链路调用真实 `/lucky`，只用于解释组件顺序和一次订单的状态推进。
- 库存争抢由常驻 Runner 精确生成 600 个不同 uid 并同时放行，调用真实 `/lucky`。每轮先重置
  订单、Redis admission、300 份库存、指标和令牌桶；600 小于默认满桶容量 800，因此限流数必须为 0。
  Runner 在请求结束后保留 5 秒观察窗记录普通 `CREATE_ORDER` 的消费进度，但不要求积压立即归零；
  积压只表示尚未完成的落单工作，不足以单独证明削峰效果。最后冻结准入、售罄、库存、系统错误、
  普通消息入队/消费/积压和超卖结论。如果运行时限流配置介于 1..599，Runner 会拒绝启动这轮实验，
  避免保护线成为隐藏变量；0 表示关闭限流，也允许运行库存实验。
- 入口限流使用 `/api/seckill/rate-limit-probe`。探针和 `/lucky` 共用同一个进程内令牌桶，获得令牌后
  立即返回 204，被拒绝时返回 429；它不读取 Redis/MySQL、不发送 MQ、不创建订单。Runner 只允许
  固定 1500 QPS 并运行 10 秒，页面要求当前阈值为 800 QPS；429 属于预期限流结果，不能计入系统错误率。
  当前两项任务准备时均调用完整实验重置；“不扣库存”仅描述探针请求本身。

库存实验使用定量并发 HTTP 发生器而不是持续 QPS；限流实验使用 wrk2 固定到达率。两者仍复用
Runner 的全局单任务锁、任务状态持久化、停止语义和 SSE 转发，因此与右侧查询压测不能并行运行。
当前令牌桶是单进程保护，实验结论不能外推为多实例全局限流。

页面主结果中的 `luaAdmissionSuccess` 在现有 `RecordRedisPreDeduct` 记录点计数，只代表 Lua 返回 OK；
后续失败回补不撤销计数。`queueSuccess` / 旧任务 `admissionSuccess` 仍是准入并入队口径，不能替代它。
旧任务缺少独立 Lua 计数时页面显示缺失并要求重跑，不输出无超卖结论。`createOrderConsumed` 只计首次
落单并推进待支付，既不是支付成功数，也不是直接查询 MySQL 的订单总数。库存结论仅针对本轮准入；
不覆盖支付、取消或超时回补。限流平均速率使用 Runner 的实测时长，结论允许一秒满桶容量带来的突发。
...
上级 `/material-shop?experiment=purchase` 是静态实验导览：并排说明 A/B 路径、观察指标和固定条件，不在这里选择运行方案或发起购买。
导览入口携带 `intent=new`；即使没有预选策略，也先进入准备状态，历史对比记录保留，只有用户明确开始实验才重置库存并执行购买。

购买页的“重置实验”单独调用现有 reset 接口：成功后清空本轮实时状态、探针、回放游标、A/B 历史结果（内存与 sessionStorage）及工程证据，保留所选方案，回到干净的准备状态，不调用 run、不影响查询实验结果。结果区域隐藏且旧表格和 Trace 内容清空，刷新不会恢复旧结果。运行或重置期间禁用重置、开始、方案切换和回放入口；重置失败不伪造基线成功，保留此前观测记录并提示重试。开始新一轮或再跑另一方案仍保留 A/B 对比，只有显式“重置实验”清场。
...
默认停在六步时间线的最终结果，由用户选择回看或播放；同步与异步方案共用嵌入执行流顶部的回放控制条，
...
六阶段由 `purchase-stages.js` 同时驱动实时状态和回放：A 为请求、MySQL 提交、DEL、响应、查询回源/重建、最终检查；
B 只交换响应与 DEL 的顺序。同步删除取 Trace 和 `cacheInvalidatedAt`，异步删除取 Outbox 的 `completed/invalidatedAt`，
发布成功不算删除完成。删除阶段的“未缓存”是 DEL 事件的语义后态，不伪装成当前物理快照；实时库存仍读取 State/GetRun。
查询样本透传原 Cached 路径已有的 `cacheRebuilt/cacheRebuildFailed`，并记录服务端开始、完成时间；
只有最后一次 DEL 之后开始的 `redis-miss` 且 SET 成功，才确认第 05 步。旧记录缺少此证据时显示“未观测”，
不会用 HIT、Redis 降级或最终相等补造回填。第 06 步在最终查询和 State 对照收尾后展示库存与冻结探针统计。
160ms 轮询可能跨过短阶段，实时不为展示而拖慢实验或补播；回放按证据逐项查看。游标升级为 v3，旧五步游标不复用，历史结果保留。
...
新请求在 Lua 准入前生成随机 `order_id`，读取持久的 `run_id`；Lua 同时扣库存并写入 `giftID|state|order_id|run_id`。普通落单、延迟取消消息及支付/取消请求携带这两个标识。消费者、Redis 状态迁移和 MySQL 查询均核对同一订单，MySQL 业务订单号有唯一索引；自增主键只作账本内部定位。

实验重置更新轮次，普通服务重启保留轮次。旧轮次和无标识的旧消息不能匹配新资格，会被忽略并确认消费；重复落单仅首次推进状态计入落单数，重复取消不再次回补。消息积压指标也按轮次过滤。历史无标识账本保留兼容处理。

当前单实例使用进程内读写屏障，避免重置插入消息的资格校验与账本写入之间。多实例部署还需要跨实例的重置协调；本改动不解决 Redis 准入成功后、MQ 发送前进程崩溃的可靠投递缺口。
