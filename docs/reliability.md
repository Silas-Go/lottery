...
平级配置入口，不再经过单次查询或第二层方式选择。查询分支用两张可切换的解说卡介绍 Direct 与 Cache-Aside 的工程行为、缓存击穿/穿透风险；卡片只切换右侧方案说明，不代表运行路径已经选定。配置页同时设置两轮共同的查询潮汐、通路模式和固定时长，真正的本轮读取路径仍在实验室选择。该页面只保存查询
计划并转场，不调用任务创建接口，也不重置购买实验库存。自动连接数在配置页只以 Direct 作为预估请求样本，进入实验室选定路径后会用同一估算接口重新计算，任务创建时才锁定最终值；手动连接数则作为两条路径共享条件直接带入。
