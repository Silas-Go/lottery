...
`/seckill-lab` 只启动一次固定批次：1500 个唯一用户同时发起真实 `/lucky`，依次经过进程内令牌桶、Redis Lua 准入和 MQ 异步落单。默认入口为 1200 QPS（满桶容量 1200），库存为 1000。Runner 沿用 `seckill-stock-burst` 任务类型和全局单任务锁，每轮重置订单、资格、库存、轮次和指标；参数基线不匹配时拒绝发压。旧版独立限流探针仍兼容 API 和历史任务，页面不再启动它。

Runner 先建立连接，再统一发送请求，避免建连耗时将洪峰摊平；准备阶段不发送业务请求。1500 是请求总数，1200 QPS 是令牌补充速率；到达时间和补充令牌影响实际放行，不能预设为 300 个限流、200 个库存不足。页面核对总请求 = 被限流 + 放行，放行 = Lua 准入成功 + 业务拒绝，并要求 HTTP 429 与限流数一致、库存为零、准入及入队均为 1000、无 HTTP/服务端异常，才展示本轮库存准入未超卖。限流或库存拒绝未发生时单独说明“未完整展示两次筛选”，不伪造结论。

`luaAdmissionSuccess` 是独立 Lua 成功计数，后续回补不会撤销；`queueSuccess` 不能替代它。未获资格包括库存不足、重复参与等业务拒绝，不表示每次拒绝都进入过 Lua。`createOrderConsumed` 是首次进入待支付的数量，不是支付成功数。请求完成后最多观察普通落单 5 秒并冻结本轮结果，MQ 消费继续进行；积压不能单独证明削峰效果。页面同时保留真实单次请求及支付入口，支付或放弃后返回实验室。

动画只示意请求向下流动，不作为分段追踪或指标来源。该实验展示单实例下的请求筛选与库存准入正确性，不构成稳定 QPS 承载能力基准，不覆盖支付、取消、超时回补的完整正确性，也不能外推为多实例全局限流。
