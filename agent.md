...
本项目是秒杀链路演示系统，注释必须服务于“讲清楚为什么这样设计”，不要机械解释代码表面含义。
所有新增或修改的复杂逻辑，禁止只提交代码而没有设计说明。
...
这类注释没有价值，因为它只是把代码表面含义重复了一遍。

推荐写法：

```go
// 使用游标分页而非 offset 分页，避免消息量增大后出现深分页性能问题。
messages, err := repo.ListMessages(...)

// 消息入库与会话摘要更新必须放在同一个事务中，
// 防止服务异常退出时出现消息存在但会话列表未更新的问题。
```
...
- 涉及多步业务流程时，必须先描述流程，再写代码。
- 流程注释用于帮助维护者快速建立全链路模型，而不是逐行解释实现。
...
- Redis 操作必须说明它在链路中的职责边界。
- 秒杀场景尤其要说明原子性、库存一致性、重复参与、超时释放等问题。
...
// Redis 仅负责预扣库存。
// 最终抽奖结果以数据库记录为准。
// 即使 Redis 重启导致缓存丢失，也不会影响最终中奖数据。
```

事务注释规范：

- 数据库事务必须说明哪些操作被绑定在一起。
- 注释要解释不放在同一事务中会产生什么数据不一致。
