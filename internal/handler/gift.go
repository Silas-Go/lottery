...
// GiftHandler 处理限量材料展示和高并发抽取 HTTP 请求。
...
// handler 通过依赖注入拿到 Redis 异步模式，保持路由层和业务层解耦。
...
// GetAllMaterials 返回秒杀/抽取实验使用的限量材料目录。
// 展示列表不承担库存真实性保证，真实可抢库存会在准入时重新从 Redis 或 MySQL 读取。
func (h *GiftHandler) GetAllMaterials(ctx *gin.Context) {
	start := time.Now()
	slog.Info("get seckill materials request start", "client", ctx.ClientIP())

	materials, appErr := h.lottery.ListMaterials()
	if appErr != nil {
	...
	if err != nil {
		writeAPIError(ctx, http.StatusInternalServerError, "GIFT_JSON_ENCODE_FAILED", "限量材料目录序列化失败", err)
		return
	}

	slog.Info("get seckill materials request success", "count", len(materials), "duration_ms", time.Since(start).Milliseconds())
