...
const materialDetailCachePrefix = "archive:material-detail:v1:"
...
func (MaterialCatalog) TableName() string { return "material_catalog" }
...
func (MaterialTrade) TableName() string { return "material_trades" }
...
func (MaterialReview) TableName() string { return "material_reviews" }
...
var defaultMaterialCatalog = []MaterialCatalog{
	{ID: 1, Code: "ARC-001", Name: "月盐", Title: "月潮退去后留下的低温结晶", Sigil: "Ⅰ", Accent: "#68cfd1", Summary: "稳定、常见且易于计量的炼成介质。", Oath: "潮水会退去，月盐会留下。", Price: 90, Stock: 64, RarityID: 1, SourceID: 1, Attribute: "低温稳定 · 吸热", Usage: "炼成介质与温控缓冲", Risk: "过量使用会造成局部低温脆化。"},
	{ID: 2, Code: "ARC-002", Name: "雾银", Title: "能在雾中保持镜面反射的液态银", Sigil: "Ⅱ", Accent: "#83a9c9", Summary: "适合感应器与镜面术式的稀有导体。", Oath: "雾遮住道路，银仍记得光。", Price: 360, Stock: 48, RarityID: 2, SourceID: 2, Attribute: "折射 · 液态金属", Usage: "镜面术式与感应组件", Risk: "强魔力场中形态不稳定，需隔离保存。"},
	{ID: 3, Code: "ARC-003", Name: "龙息琥珀", Title: "封存古老高温吐息的动力核心", Sigil: "Ⅲ", Accent: "#d08a57", Summary: "持续释放热能，常用于高负荷炼成装置。", Oath: "火焰沉睡，但从未熄灭。", Price: 1280, Stock: 24, RarityID: 3, SourceID: 3, Attribute: "高温封存 · 持续放能", Usage: "动力核心与耐热封装", Risk: "高温或撞击可能触发能量泄漏。"},
	{ID: 4, Code: "ARC-004", Name: "星髓", Title: "从坠星内部提取的高密度魔力介质", Sigil: "Ⅳ", Accent: "#8877d7", Summary: "只在高阶炼成中使用的稀缺校准材料。", Oath: "群星沉默，星髓仍在迁移。", Price: 5200, Stock: 12, RarityID: 4, SourceID: 4, Attribute: "高密度魔力 · 星光迁移", Usage: "高阶炼成与能量校准", Risk: "高密度魔力会干扰未经屏蔽的仪器。"},
}

var defaultMaterialComponents = []MaterialComponent{
	{ID: 101, MaterialID: 1, Name: "霜晶粉", Quantity: 6, Unit: "g", SortOrder: 1},
	{ID: 102, MaterialID: 1, Name: "月潮水", Quantity: 12, Unit: "ml", SortOrder: 2},
	{ID: 103, MaterialID: 1, Name: "盐沼草灰", Quantity: 2, Unit: "g", SortOrder: 3},
	{ID: 201, MaterialID: 2, Name: "雾银砂", Quantity: 8, Unit: "g", SortOrder: 1},
	{ID: 202, MaterialID: 2, Name: "镜湖凝露", Quantity: 6, Unit: "ml", SortOrder: 2},
	{ID: 203, MaterialID: 2, Name: "导魔铜屑", Quantity: 1, Unit: "g", SortOrder: 3},
	{ID: 301, MaterialID: 3, Name: "赤脊树脂", Quantity: 10, Unit: "g", SortOrder: 1},
	{ID: 302, MaterialID: 3, Name: "龙焰灰", Quantity: 3, Unit: "g", SortOrder: 2},
	{ID: 303, MaterialID: 3, Name: "黑曜稳定片", Quantity: 2, Unit: "片", SortOrder: 3},
	{ID: 401, MaterialID: 4, Name: "坠星碎屑", Quantity: 5, Unit: "g", SortOrder: 1},
	{ID: 402, MaterialID: 4, Name: "夜空溶剂", Quantity: 9, Unit: "ml", SortOrder: 2},
	{ID: 403, MaterialID: 4, Name: "银线封印", Quantity: 4, Unit: "圈", SortOrder: 3},
}
...
func (s *Store) GetMaterialDetail(id int) (*MaterialDetailDTO, int, error) {
	...
	if err := s.db.Raw(`
		...
		WHERE material_id = ?
		ORDER BY sort_order, id`, id).Scan(&detail.Components).Error; err != nil {
	...
	if err := s.db.Raw(`
		SELECT
			...
			COALESCE(SUM(CASE WHEN traded_at >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL 24 HOUR) THEN 1 ELSE 0 END), 0) AS transactions24h,
